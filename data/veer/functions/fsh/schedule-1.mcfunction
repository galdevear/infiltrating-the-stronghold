...
tag @s[tag=veer.fsh.new] remove veer.fsh.new
tag @s[tag=!veer.fsh,tag=!global.ignore,tag=!global.ignore.pos] add veer.fsh.new
tag @s[tag=!veer.fsh,tag=!global.ignore,tag=!global.ignore.pos] add veer.fsh

#> Silverfish
# Also includes silverfish queen endermite spawning. (any silverfish in stronghold)
execute if entity @s[type=silverfish, tag=veer.fsh.new] run function veer:fsh/silverfish/init

#> Watcher Spawning
# Swap some new spawns with a pair of watchers
execute if entity @s[type=#veer:fsh/watcher_pair/replaceable,tag=veer.fsh.new,predicate=veer:fsh/in_stronghold] run function veer:fsh/watcher_pair/detect_replaceable
...
execute if entity @s[type=#veer:fsh/fix_glowing/passengers, tag=veer.fsh.fix_glowing.passenger] if score @s veer.fsh.fix_glowing.glowTimer matches 0.. run function veer:fsh/fix_glowing/decrement
