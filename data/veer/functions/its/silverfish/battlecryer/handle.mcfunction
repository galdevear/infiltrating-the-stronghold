...
execute if entity @s[type=silverfish,tag=veer.its.silverfish.battlecryer.retToY,scores={veer.its.silverish.battlecryer.lifeTimer = 0}] run execute store result entity @s Pos[1] double 1 run scoreboard players get @s veer.its.silverfish.battlecryer.ay
...
execute if entity @s[type=silverfish, scores={veer.its.silverish.battlecryer.lifeTimer=19..}] run function veer:its/util/kill_in_void
# Increment timer
